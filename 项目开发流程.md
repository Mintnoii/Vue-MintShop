...
1. 商家列表是位于首页轮播图下面的部分，可以拆分为一般组件，所以在components文件夹下新建ShopList文件夹并新建ShopList.vue文件
2. 该组件中用到需要一些图片资源，所以在ShopList文件夹下还需要新建images文件夹，然后将原本放在Msite文件夹下的shop和stars图片资源移动到新建的images文件夹内   (其实都应该是动态地从后台获取)
3. 将Msite.vue模板中的`<div class="shop_container"></div> `部分及相应的stylus样式代码移动到新建的ShopList.vue组件
4. 注意还要引入公共的css代码mixins.styl 
5. 然后在Msite.vue中import引入商家列表组件并注册使用

...
1. 登录组件为一级路由组件，所以在pages文件夹下新建此组件文件

2. template模板里会用到一张svg图片(静态的验证码图片)，所以还要在Login文件夹下新建images文件夹

3. 将路由组件映射为路由，在router下的index.js文件里进行配置

4. 登录组件的路由是从个人中心Profile组件里跳转而来的，所以要修改Profile.vue
...
5. 然后编写Login.vue组件的template和style代码

6. 注意到一个问题，在一个路由组件(Msite)将页面下拉，再切换到其他路由组件(Profile)，页面不会自动回到顶部。
